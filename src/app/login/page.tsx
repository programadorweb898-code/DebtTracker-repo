...
export default function LoginPage() {
  ...
  const loginEmailValue = loginWatch('email');
  const loginPasswordValue = loginWatch('password');
  
  useEffect(() => {
    if (loginEmailValue && loginErrors.email?.message === 'El campo email es requerido.') {
      loginClearErrors('email');
    }
    if (loginPasswordValue && loginErrors.password?.message === 'El campo contraseña es requerido.') {
      loginClearErrors('password');
    }
  }, [loginEmailValue, loginPasswordValue, loginErrors, loginClearErrors]);
  ...
  const { 
    ...
    clearErrors: forgotPasswordClearErrors 
  } = forgotPasswordForm;
  const forgotPasswordEmailValue = forgotPasswordWatch('email');

  useEffect(() => {
    if (forgotPasswordEmailValue && forgotPasswordErrors.email?.message === 'El campo email es requerido.') {
      forgotPasswordClearErrors('email');
    }
  }, [forgotPasswordEmailValue, forgotPasswordErrors, forgotPasswordClearErrors]);
    ...
    <Dialog open={isResetDialogOpen} onOpenChange={handleDialogClose}>
      <div className="flex min-h-screen items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            <Form {...loginForm}>
              <form noValidate onSubmit={loginForm.handleSubmit(onLoginSubmit)} className="space-y-4">
                <FormField
      ...
      <DialogContent>
          ...
          <form noValidate onSubmit={forgotPasswordForm.handleSubmit(onForgotPasswordSubmit)} className="space-y-4">
              <FormField
